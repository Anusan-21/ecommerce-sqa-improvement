...
context("Sign In page", () => {
  // Hardcoded for your environment
  ...
  it("submits and navigates on successful sign in", () => {
    // Exact endpoint used by RTK Query: /auth/sign-in
    ...
    // type and press enter to ensure form submission via keyboard also works
    cy.get('input[placeholder="Password"]').type("password123{enter}");

    // give the app a bit more time to perform the network call and redirect
